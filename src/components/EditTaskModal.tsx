  ...
  React.useEffect(() => {
    if (isOpen && task) {
      ...
      setFormData({
        ...
        assignedToId: task.assignee?.id?.toString() || "",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (formData.assignedToId) {
        ...
        submitData.assignedToId = null;
      }

      await onSubmit(submitData as UpdateTaskData);
      onClose();
    } catch (error: any) {
      if (error?.response?.data?.errors) {
        ...
        setErrors({ submit: error?.response?.data?.message || error?.message || "Failed to update task" });
      }
    } finally {
